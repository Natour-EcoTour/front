...
import { Map, Eye, MapPinnedIcon } from 'lucide-react';
...
export function PendingPointsTable() {
    ...
    const refreshPoints = useCallback(async () => {
        setIsLoading(true);
        try {
            const searchParams: { page: number; name?: string; status: string } = { page: currentPage, status: 'null' }
    ...
    const confirmReprove = async () => {
        if (selectedId) {
            await changePointStatus(selectedId, { status: false, is_active: false })
                .then(() => {
                    setShowReproveSuccessModal(true);
                    refreshPointsSilently(); // Refresh the table silently after successful reproval
                })
                .catch((error) => {
            ...
            <div className="overflow-hidden rounded-lg border-2 border-green-200">
                <table className="min-w-full table-auto">
                    <thead className="bg-gray-200">
                        <tr>
                            {['Nome', 'Link', 'Localização', 'Detalhes', ''].map(h => (
                            ...
                            return (
                                <tr key={point.id} className="hover:bg-green-50">
                                    ...
                                    <td className='space-x-3'>
                                        ...
                                        <button
                                            onClick={() => handleReproveClick(point.id.toString())}
